...
//                      Fixed "Send Message" to Page button if "full screen chat" option is turned on, fixed hiding new games crap in right column, added numerals to the font selector preview popup text, removed old trending related code, added option to hide business discovery left column link
...
'<tr title="Hide the animations that are shown when certain words/phrases are typed in posts or comments such as rad,best wishes,xoxo,you got this,bff"><td><label class="fbpoptslabel" for="textanimations">Hide Text Animations</label></td><td><input id="textanimations" type="checkbox"></td></tr>'+
...
'<tr><td></td><td><b></b></td></tr><tr style="background-color:#ECEFF5 !important" title="Shows the Logout button in the top navigation bar"><td><label for="topnavbarlogout" class="fbpoptslabel">Show Log Out button</label></td><td><input id="topnavbarlogout" type="checkbox"></td></tr>' +
...
// comment this section out for testing offline pages
 if(currentuserid=="") {
  ...
  loginandoutstyle.textContent='div.fbIndexFeaturedRegistration > div.lfloat, .-cx-PRIVATE-fbAdUnit__root, #pagelet_rhc_footer, #SetAsHomepage_Callout, .gradientContent > div[data-ad^="{"], #findFriendsNav, #navFindFriends, div[data-ad], body.UIPage_LoggedOut #globalContainer div.lfloat, body.UIPage_LoggedOut div._hlg, #pagelet_growth_expanding_cta {display:none} body.UIPage_LoggedOut #globalContainer div.userContentWrapper div.lfloat,#reg_box .lfloat._ohe {display:block !important}';
...
//  Your memories
  ...
  if (fbpoptsobj.yourmemories ==1)
	extraTextFilter+= ",Your Memories on Facebook,Years? of Friendship on Facebook,Celebrate Your Friendship With,We made you this video to celebrate your friendship,see more memories,A Look Back at Your Summer,we made you a video to look back on,your year in review";
...
// top nav background image
  ...
  if(fbpoptsobj.colouredbackgsquares=="undefined") {
    ...
    fbpfreestyle.textContent +=' #feedx_sprouts_container table[cols="8"],#feedx_sprouts_container table[cols="9"],#feedx_sprouts_container table[cols^="1"],#timeline_composer_container table[cols^="1"],#timeline_react_composer_container table[cols="8"]{display:none}';
  }
  // hide searchbox
  ...
  for(var b=0;b<lcoptsarr.length;b++)
	...
	if(fbpoptsobj.colouredbg==1) // neutralise coloured status updates
	  ...
	  'div._5qxm, div._5qxm div {background-image:inherit !important;background-color:inherit !important}' +
...
var fbpdynamicreload = function (opts) {
      ...
      for(q=0;q<records.length;q++) {
	    ...
	    for(p=0;p<records[q].addedNodes.length;p++) {
          ...
          if((tempnode.nodeName=='LI') ||  (tempnode.nodeName=='UL')) {
	        ...
	        if (document.getElementById('pagelet_litestand_section') || document.getElementById('stream_pagelet') || document.getElementById('pagelet_explore_feed'))  {
	          //console.log(event.target.nodeName);
  ...
  function cleartheshizzle(thenode) {
	...
	if(!(oldfeedstyle || newfeedstyle || neweststyle || explorestyle) ) {
...
*/
  function fbpsharer() { 
	...
	var fbpsharemsg="Want more control over what you see on Facebook? F.B. Purity is a safe, free and top rated browser add-on that lets you clean up the junk on Facebook and customize it to make it just the way you like it. F.B. Purity has over 437,000 users worldwide and has been reviewed and highly recommended by The Washington Post, The New York Times, CNET, PCWorld, MacWorld and many other top tech journals. Get F.B. Purity free via FBPURITY .COM or the F.B. Purity Fan Page on Facebook";
