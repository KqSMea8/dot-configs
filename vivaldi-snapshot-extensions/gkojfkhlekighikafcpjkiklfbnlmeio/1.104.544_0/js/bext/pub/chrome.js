...
define(['underscore', '/util/etask.js', '/bext/pub/backbone.js',
    '/util/zerr.js', '/util/version_util.js', '/util/util.js',
    '/util/user_agent.js'],
    function(_, etask, be_backbone, zerr, version_util, zutil, user_agent){
...
function on_connect(cid){
    // XXX shachar/bahaa: removed assert until issue resolved (from FF45 this
    // happens consistently whenever tpopup opens), need to debug why exactly
    if (E.listeners[cid])
...
E.impl.init = function(){
    ...
    if (!chrome)
        return;
    if (version_util.cmp(user_agent.guess_browser().version, '23')<0)
    {
...
E.impl['be.reload_ext'] = chrome && function(args, cb, j){
    ...
    // for firefox location.reload() doesn't reload the extension
...
E.impl.add_listener = function(obj, sub, cb, opt, extra){
    try {
        ...
        // XXX bahaa FF_WEBEXT: review
        if (conf.firefox_web_ext && !opt)
