...
var user_status_model = Backbone.Model.extend({
    ...
    refresh: function(){
        var _this = this;
        return etask([function(){
            return ajax.hola_api_call('users/get_user?source=be_user_nav');
        }, function(res){ _this.update(res.user); }]);
    },
    ...
    on_logout: function(){
        ...
        return etask([function(){
            return ajax.hola_api_call('users/logout/inline', {method: 'POST',
                text: true});
        }, function(){ return E.be_premium.refresh_membership(); },
        function(){ _this.model.refresh(); }]);
    },
});
