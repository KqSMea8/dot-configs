...
**************************************************************************************************/
var buildReplacementHTML = function(identifiedAd, pageURL) {
	...
	//determine if vertical whitespace should be collapsed
	var collapseVerticalWhitespace = (pageURL.indexOf('facebook.com') >= 0);
	...
	intentlyData.channelResources.forEach(function(channelResource) {
		...
		if(sizeConstraintsPassed && historyConstraintsPassed && adminIntentionConstraintsPassed) {
			...
			intentlyData.channelResourceHistory[channelResource.id]++;
		}
	});
	...
	if(candidateResources.length > 0) {
		...
		for(var lcv = 0; lcv < candidateResources.length; lcv++) {
			...
			candidateResources[lcv].historicalScore = intentlyData.channelResourceHistory[candidateResources[lcv].id] * 100 / intentlyData.channelResources.length;
		...
		if(Conf.DEBUG) {
			console.log('----------------------------------------------------------------------------');
			console.log('weighted scores for ad ' + identifiedAd.intentlyID + ' (' + identifiedAd.originalWidth + ' x ' + identifiedAd.originalHeight + ' = ' + adOriginalPixelArea + '):');
		}
		*/
		...
		if(highestScoreIndices.length > 0) {
			...
			for(lcv = 0; lcv < candidateResources.length; lcv++) {
				if(candidateResources[lcv] != selectedIndex) {
					intentlyData.channelResourceHistory[candidateResources[lcv].id]++;
				}
			}
			//update history usage for selected resource
			intentlyData.channelResourceHistory[candidateResources[selectedIndex].id] = 0;
	...
	if(replacementResource) {
		...
		//build the main div
		replacementHTML += '<div ';
		replacementHTML += 'intentlyReplacementID="' + identifiedAd.intentlyID + '" ';
		...
		replacementHTML += 'height: ' + (collapseVerticalWhitespace ? resourceHeight : containerHeight) + 'px; ';
		replacementHTML += 'width: ' + containerWidth + 'px; ';
		replacementHTML += '">';
		//build the positioning div
		replacementHTML += '<div ';
		replacementHTML += 'intentlyReplacementParentID="' + identifiedAd.intentlyID + '" ';
		...
		replacementHTML += '<div ';
		replacementHTML += 'intentlyReplacementParentID="' + identifiedAd.intentlyID + '" ';
		replacementHTML += 'class="intentlyImpressionOverlay" ';
		...
		replacementHTML += '<img ';
		replacementHTML += 'id="thumbsUpImpressionButton-' + identifiedAd.intentlyID + '" ';
		replacementHTML += 'intentlyReplacementParentID="' + identifiedAd.intentlyID + '" ';
		replacementHTML += 'channelResourceID="' + replacementResource.id + '" ';
		...
		replacementHTML += 'activeClass="intentlyImpressionButton" ';
		...
		if(replacementResource.impression === true) {
			...
			replacementHTML += 'class="intentlyImpressionButton" ';
		...
		replacementHTML += '<img ';
		replacementHTML += 'id="shareIntentionButton-' + identifiedAd.intentlyID + '" ';
		replacementHTML += 'class="intentlyShareButton" ';
		replacementHTML += 'intentlyReplacementParentID="' + identifiedAd.intentlyID + '" ';
		replacementHTML += 'channelResourceID="' + replacementResource.id + '" ';
		replacementHTML += 'style="position: absolute; ';
		...
		replacementHTML += '<img ';
		replacementHTML += 'id="thumbsDownImpressionButton-' + identifiedAd.intentlyID + '" ';
		replacementHTML += 'intentlyReplacementParentID="' + identifiedAd.intentlyID + '" ';
		replacementHTML += 'channelResourceID="' + replacementResource.id + '" ';
...
var openIntentionReviewPage = function(imageURL, pageURL, description) {
	...
	var handleIntentlyGlobalMsg = function (tabID,messageData,callback) {
 ...
 * Returns: None.
 **************************************************************************************************/
var handleIntentlyMsg = function (tabID,messageData,callback) {
	...
	if (!messageData )
		return;
			...
			if (!extension.intentlyActive )					
				return handleIntentlyGlobalMsg(tabID,messageData,callback);
				
	//console.log("handleIntentlyMsg",tabID,messageData)
...
//Load definitions is the first call , check not whitelisted
	if(messageData.messageType == 'loaddefinitions' && µb.getNetFilteringSwitch(tabURL) == true ){	
		...
		activeReplacementOperations[tabID].identifiedAds.forEach(function (identifiedAd) {
			...
			identifiedAd = buildReplacementHTML(identifiedAd, messageData.pageURL);
		...
		if (messageData.response == 'adsreplaced' && activeReplacementOperations[tabID]) {
			...
			if (activeReplacementOperations[tabID].replacedAds && activeReplacementOperations[tabID].replacedAds.length > 0) {
			...
			if (messageData.lastRightClickAd) {			
					...
					success : function (result, status, xhr) {
						...
						if (result.success === true) {
							...
							var notificationOptions = {
								type : 'basic',
								title : 'Intently Ad Identified',
								message : 'Thank you for your help in making the Internet a more fulfilling place! The ad has been successfully reported to the Intently system for verification.',
								alert : 'Thank you for your help in making the Internet a more fulfilling place! The ad has been successfully reported to the Intently system for verification.',
								iconUrl : 'img/browsericons/intentlyLogo128.png'
							};
							issueAlert('intentlyAdIdentifySuccess', notificationOptions);
						} else {
							//something went wrong, notify user
							var notificationOptions = {
								type : 'basic',
								title : 'Intently Ad Identification Error',
								message : 'Whoops! Something unexpected happened while reporting the identified ad to the Intently system. This error has been recorded and will be reviewed by the Intently technical team.',
								alert : 'Whoops! Something unexpected happened while reporting the identified ad to the Intently system. This error has been recorded and will be reviewed by the Intently technical team.',
								iconUrl : 'img/browsericons/intentlyLogo128.png'
							};
							issueAlert('intentlyAdIdentifyFailure', notificationOptions);
						}
					}
				});
			} else {
				...
				var notificationOptions = {
					type : 'basic',
					title : 'Intently Ad Not Identified',
		...
		if (messageData.action == 'impressionbuttonclicked') {
			...
			if (messageData.impressionButtonClickData) {
				...
				intentlyData.channelResourceUserImpressions.push(messageData.impressionButtonClickData);
				...
				if (messageData.impressionButtonClickData.impression === '0') {
					...
					for (var lcv = 0; lcv < intentlyData.channelResources.length; lcv++) {
						if (intentlyData.channelResources[lcv].id == messageData.impressionButtonClickData.channelResourceID) {
							intentlyData.channelResources.splice(lcv, 1);
						}
					}
				}
			}
	...
	} else if (messageData.messageType == 'adblockerdetected') {
		//create and display notification
		Utils.log('issuing ad blocker detection notification.');
		var notificationOptions = {
			type : 'basic',
			title : 'Another Ad Blocker Detected',
			message : 'Intently has detected another ad blocker installed on your browser. In order for intently to operate, please disable other ad blockers.',
			alert : 'Intently has detected another ad blocker installed on your browser. In order for intently to operate, please disable other ad blockers.',
			iconUrl : 'img/browsericons/intentlyLogo128.png'
		};
		issueAlert('adblockerdetected', notificationOptions);
	}
}


//Cross channel communication
vAPI.messaging.handleIntentlyMsg = function (tabID,messageData,callback) {

	return handleIntentlyMsg(tabID,messageData,callback)
}

var onMessage = function(request, sender, callback) {

	//console.log("Intently content",request,sender,callback)
    ...
    switch ( request.what ) {
		case "intently":
					 if ( sender && sender.tab ) 
							handleIntentlyMsg(sender.tab.id,request,callback)	
					else
...
vAPI.messaging.listen('intently-content.js', onMessage);
...
var onMessage = function(request, sender, callback) {
    ...
    switch ( request.what ) {
		case "intently":
					 if ( sender && sender.tab ) {
						// console.log("received message from intently client")
...
vAPI.messaging.listen('intently-client.js', onMessage);
