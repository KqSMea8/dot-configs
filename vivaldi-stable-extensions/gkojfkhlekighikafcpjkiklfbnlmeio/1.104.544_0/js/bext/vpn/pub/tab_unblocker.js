...
function _on_headers_received(details, host){
    ...
    // XXX: reuse (move logic to shared function): use direct_first
...
function _on_before_request(details, host, route_str, opt){
    ...
    // XXX: move hosts caching to shared logic
    var host_cache = E.hosts_cache[host];
    if (!req)
    {
        req = E.requests[details.requestId] = {
...
E.init = function(){
    ...
    new_unblocker_api = !be_features.have(be_ext, 'old_unblocker_api');
    ...
    if (zutil.is_mocha())
        hola_req_id = 0;
    be_lib.perr_ok({id: new_unblocker_api ? 'new_unblocker_api_init' :
        'old_unblocker_api_init'});
};
