...
{
	userLoggedIn : true,
	intentlyActive : false,
	notificationCount:0,
	
	intentlyContextMenus :
	{
		mnuIntentlyContext : null,
		//TODO can be removed
		...
		handleAddIntentions : function (details, tab)
		{
			...
			var script = "" +
				...
				"							url:document.URL" +
				"						};" +
				"				  intentlyContent.backgroundPort.onMessage(getLastRightClickInfoMessageData);"
		...
		extension.intentlyContextMenus.refresh({"id":tabId,"url": tabContext.normalURL})
		...
		refresh : function (tab)
		{
			...
			if (!extension.intentlyActive)
			{
				Utils.log("Intently not active")
				return;
			}
			 ...
			 if (tab.url && tab.url.indexOf("about") == 0) {
			  ...
			  console.log("Attempting to add menu mnuIntentlyContext")
			
			var tabDomain = Utils.parseUri(tab.url).hostname;
			var isWhitelisted = (Conf.USER_DOMAIN_WHITELISTS.indexOf(tabDomain) >= 0)
			
			var mnuIntentlyContext =
			{
				id : 'mnuIntentlyContext',
				title : 'Intently',
				isParent : true,
			...
			vAPI.contextMenu.create(mnuIntentlyContext);
			
			var mnuIntentions =
			{
				id : 'mnuIntentions',
				title : 'Add to Intentions',
				parentId : 'mnuIntentlyContext',
				contexts : ['image'],
				documentUrlPatterns : ['https://*/*', 'http://*/*']
			};
			
			vAPI.contextMenu.create(mnuIntentions, extension.intentlyContextMenus.handleAddIntentions);
			...
			vAPI.contextMenu.create(mnuChangeWhitelist, extension.intentlyContextMenus.handleWhitelistChange);
	...
	handlebrowserAction:function(){
		var url=extension.hasIntentlyTab()
...
handlebackgroundrequest : function (details)
{
	...
	var blocked = false
		//ensure intently is active and frame should be tracked for blocking
	...
	var frameDomain = details.rootHostname
		...
		|| elType == Utils.ELEMENT_TYPES.xmlhttprequest || elType == Utils.ELEMENT_TYPES.subdocument
			...
			blocked = intentlyData['adBlockingRules'].matches(details.url, elType, frameDomain);
...
var status = "off"
		var label='You are not logged in to Intently! Click the button to log in.'
		Utils.log("checking logging in",extension.userLoggedIn)
		var badge = ""
	if (extension.userLoggedIn) {

		if (extension.intentlyActive &&  !extension.intentlyInitializing ) {
			status = "active"
			 ...
			 var isWhitelisted = (Conf.USER_DOMAIN_WHITELISTS.indexOf(tabDomain) >= 0)
				...
				if (extension.notificationCount > 0) {
						badge=extension.notificationCount +""
							label = 'You have Intently notifications! Click here to open the Intently Dashboard and view your notifications.'
	...
	detail.label=label
        ...
        vAPI.setIntentlyIcon(tabId, detail);
		Utils.log("Set Intently icon completed",detail)
},
	...
	extension.ajax(
	{
		...
		success : function (result, status, xhr)
		{
			if (result.success)
			{
				...
				extension.intentlyContextMenus.refresh(tab)
					...
					var notificationOptions =
				{
					...
					iconUrl : 'img/browsericons/intentlyLogo128.png'
				};
				issueAlert('addWhitelistSuccess', notificationOptions);

				//special re-initialization case for safari since we cannot easily read cookies
				if(Conf.BROWSER_NAME == 'Safari') {
					OnRefreshCookies(function() {});
				}
			...
			else
			{
				...
				var notificationOptions =
				{
					...
					message : 'Whoops! Something unexpected happened while adding ' + tabDomain + ' to your whitelist. Please try again, or contact our support team at admin@intently.com.',
					alert : 'Whoops! Something unexpected happened while adding ' + tabDomain + ' to your whitelist. Please try again, or contact our support team at admin@intently.com.',
					iconUrl : 'img/browsericons/intentlyLogo128.png'
				};
				issueAlert('AddWhitelistSuccess', notificationOptions);
			}
			...
			success : function (result, status, xhr)
			{
				...
				else
				{
					...
					var notificationOptions =
					{
						type : 'basic',
						title : 'Whitelist Error',
						message : 'Whoops! Something unexpected happened while removing ' + tabDomain + ' from your whitelist. Please try again, or contact our support team at admin@intently.com.',
						alert : 'Whoops! Something unexpected happened while removing ' + tabDomain + ' from your whitelist. Please try again, or contact our support team at admin@intently.com.',
						iconUrl : 'img/browsericons/intentlyLogo128.png'
					};
...
init : function ()
{
	extension.intentlyInitializing = true;
	Utils.log("Initializing Intently")
	...
	initApiData(function (data)
	{
		...
		//init the extension
		extension.initIntentlyExtension();
		extension.intentlyInitializing = false;
...
initIntentlyExtension : function ()
{
	
	Utils.log('Initializing extension');
	
	extension.intentlyActive = true;
...
getAdDefinitions : function (tabURL)
{
	...
	if (domainMatches && domainMatches.length > 1)
	{
		...
		//get the domain specific ad definitions
		var domainIdentifiers = (intentlyData['adReplacementRules'][domain] && intentlyData['adReplacementRules'][domain].identifiers) ? intentlyData['adReplacementRules'][domain].identifiers : [];
		var domainExceptions = (intentlyData['adReplacementRules'][domain] && intentlyData['adReplacementRules'][domain].exceptions) ? intentlyData['adReplacementRules'][domain].exceptions : [];
		var domainHideOverrides = (intentlyData['adReplacementRules'][domain] && intentlyData['adReplacementRules'][domain].hideOverrides) ? intentlyData['adReplacementRules'][domain].hideOverrides : [];
		...
		var adDefinitionsMessageData =
		{
			...
			adDefinitions :
			{
				identifiers : intentlyData['adReplacementRules']['*'].identifiers.concat(domainIdentifiers),
				exceptions : intentlyData['adReplacementRules']['*'].exceptions.concat(domainExceptions),
				hideOverrides : intentlyData['adReplacementRules']['*'].hideOverrides.concat(domainHideOverrides)
			},
					...
					success: function(result, status, xhr) {
						...
						if(xhr.status != 200 || !xhr.json || !xhr.json.success) {
							...
							//deactivate extension operation
								...
								//deactivate extension operation
							extension.intentlyActive = false;
							...
							var notificationOptions = {
								type: 'basic', 
								title: 'Intently Login', 
								message: 'You are currently not logged in to the Intently service. Intently cannot operate without being logged in. Click on the Intently icon to the right of your address bar to login.',
								alert: 'You are currently not logged in to the Intently service. Intently cannot operate without being logged in. Click on the Intently icon to the right of your address bar to login.',
								iconUrl : 'img/browsericons/intentlyLogo128.png'
							};
...
/**************************************************************************************************
 * Purpose:
 *	Retrieves the extension configuration information from the Intently API and loads all
 *	subsequent data into the intentlyData object. Syncs ad definitions if they are expired
 *	and caches the global ad identifiers from  local storage into the intentlyData object.
 *
 * Params:
...
var initApiData = function (callbackFunction)
{
	...
	//ensure channel resource history object is defined
	if(!intentlyData['channelResourceHistory']) intentlyData['channelResourceHistory'] = {};
	...
	var syncAdDefinitionsCallback = function (completedResourcePurges)
	{
		...
		var cacheAdDefinitionsCallback = function (adDefinitionRetrievalStats)
		{
			...
			vAPI.storage.get('adReplacementRules', function (localStorageObject)
			{
				...
				intentlyData['adReplacementRules'] = localStorageObject['adReplacementRules'];
				...
				vAPI.storage.get('adBlockingRules', function (localStorageObject)
				{
					...
					//hardcoded whitelist for intently
					var intentlyWhitelistFilter = Filter.fromText('@@||www' + Conf.DOMAIN);
					filters.whitelist[intentlyWhitelistFilter.id] = intentlyWhitelistFilter;
					var intentlyApiWhitelistFilter = Filter.fromText('@@||api' + Conf.DOMAIN);
					filters.whitelist[intentlyApiWhitelistFilter.id] = intentlyApiWhitelistFilter;
					//process the patterns and whitelists into a filter set
					intentlyData['adBlockingRules'] = new BlockingFilterSet(
	...
	extension.ajax({
		...
		success : function (result, status, xhr)
		{
			...
			Conf.REMOTE_DATA_INIT_INFO.forEach(function (remoteDataInit) {
				...
				delete intentlyData[remoteDataInit.propertyName];
				intentlyData[remoteDataInit.propertyName] = [];
				...
				extension.ajax({
					...
					success : function (result, status, xhr)
					{
						...
						{
							//save the data
							intentlyData[remoteDataInit.propertyName] = response.data;
							//increment counter
							dataInitCounter++;
							//see if all data is initialized
							if(dataInitCounter == Conf.REMOTE_DATA_INIT_INFO.length ) {
								initStats.numOwnedChannels = intentlyData['ownedChannels'].length;
								initStats.numSubscribedChannels = intentlyData['subscribedChannels'].length;
								initStats.numSubscribedChannelResources = intentlyData['channelResources'].length;
								...
								if(intentlyData['channelResources'].length === 0) {
									//deactivate extension operation
									//intentlyActive = false;
									extension.intentlyActive = false;
									extension.intentlyInitializing = false;
									...
									var notificationOptions = {
										type: 'basic', 
										title: 'Intently Channels', 
										message: 'You are currently not subscribed to any Intently channels! Click on the Intently icon to the right of your address bar to subscribe to Intently channels.', 
										alert: 'You are currently not subscribed to any Intently channels! Click on the Intently icon to the right of your address bar to subscribe to Intently channels.', 
										iconUrl: 'img/browsericons/intentlyLogo128.png'
									};
								...
								var ownedChannelIDs = [];
								intentlyData['ownedChannels'].forEach(function(ownedChannel) {
									ownedChannelIDs.push(ownedChannel.id);
								});
								//Whitelist domains using uBlock way
								Conf.USER_DOMAIN_WHITELISTS = [];
								var whiteliststr = ""
								intentlyData['userDomainWhitelists'].forEach(function (userDomainWhitelist) {
								...
								//initialize channel resource data
								if(intentlyData['channelResources'].length > 0) {
									...
									console.log("filtering resources with negative impression");
									var channelResourcesFilteredArray=intentlyData['channelResources'].filter(filterByImpression);
									console.log("filtering resources completed")
									//move buffer array to channelResources
									intentlyData['channelResources'] = channelResourcesFilteredArray;
									...
									intentlyData['channelResources'].forEach(function(channelResource) {
										//initialize historical use tracking
										if(!intentlyData['channelResourceHistory'][channelResource.id]) intentlyData['channelResourceHistory'][channelResource.id] = intentlyData['channelResources'].length;
										//assume no honeymoon
...
var fetchChannelResourceData = function (channelResources, callbackFunction) {
	...
	if (channelResources.length > 0) {
		...
		Utils.log("setting storage");
		vAPI.storage.set(localStorageObject, function () {
			Utils.log("setting storage completed");
			//execute callback
...
var purgeChannelResourceData = function (callbackFunction) {
	...
	if (intentlyData.channelResources && intentlyData.channelResources.length > 0) {
		...
		vAPI.storage.get(null, function (localStorageObject) {
			...
			for (var key in localStorageObject) {
				...
				if (keyParts.length == 3 && keyParts[0] == 'channelResource' && keyParts[2] == 'DATA') {
					...
					var found = false;
					for (var lcv = 0; lcv < intentlyData.channelResources.length; lcv++) {
						if (intentlyData.channelResources[lcv].id == keyParts[1])
							found = true;
					}
...
Utils.log('Intently background loaded in ' + Conf.MODE + ' mode with debug ' + ((Conf.DEBUG) ? 'enabled' : 'disabled'));